...
class Mapper4 extends Mapper0 {
  ...
  write(address, value) {
    ...
    switch (address & 0xe001) {
      ...
      // Forge patch: upstream had $C000/$C001 backwards ($C000 wrote the live
      // counter, $C001 the latch). Real MMC3 takes the latch at $C000 and any
      // write to $C001 clears the counter so the next A12 clock reloads it —
      // see https://www.nesdev.org/wiki/MMC3#IRQ_Specifics. The Forge engine
      // arms its split from NMI with exactly that sequence, so the vendored
      // core has to count the way Mesen and the hardware do.
      case 0xc000:
        // IRQ latch: the value the counter reloads from
        this.irqLatchValue = value;
        break;

      case 0xc001:
        // IRQ reload: the next clock reloads the counter from the latch
        this.irqReload = true;
        break;
  ...
  // Forge patch: nesdev-correct counting. The counter runs whether or not the
  // IRQ is enabled — $E000 only gates the interrupt line — and a counter that
  // is zero (or has a reload pending) reloads from the latch instead of
  // decrementing. The IRQ asserts on the clock that leaves the counter at
  // zero, which with the "new" MMC3 behaviour includes a reload to zero.
  clockIrqCounter() {
  ...
  fromJSON(s) {
    ...
    this.irqReload = Boolean(s.irqReload);
