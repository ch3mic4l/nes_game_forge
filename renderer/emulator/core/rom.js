...
class ROM {
  ...
  getMirroringType() {
    // UNROM 512 redefines byte 6's mirroring bits: bit 3 set with bit 0 clear is
    // one-screen with the mapper choosing which, and only *both* bits set means
    // four-screen. Reading bit 3 alone as four-screen would give a one-screen
    // board four nametables. Mapper30 applies bit 7 from there.
    if (this.mapperType === 30 && this.fourScreen) {
      return this.mirroring === 0
        ? this.SINGLESCREEN_MIRRORING
