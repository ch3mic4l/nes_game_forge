...
class PaletteTable {
  ...
  // FORGE PATCH. The upstream loadNTSCPalette() table lists hues in the
  // opposite order to the canonical $xx numbering ($01 comes out red instead
  // of blue), so an emulated screen would not match what the Tile Forge and
  // Map Forge draw. Sharing one palette with the editors makes the preview
  // exact by construction.
  loadForgePalette() {
    for (let i = 0; i < 64; i++) {
      const [r, g, b] = NES_PALETTE[i];
      this.curTable[i] = this.getRgb(r, g, b);
    }
    this.makeTables();
    this.setEmphasis(0);
  }
