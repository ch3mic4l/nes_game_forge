#!/usr/bin/env node
// Writes the UNROM 512 sample project -- a durable, repeatable fixture (not a
// mkdtemp throwaway) exercising flash-backed save (engine/flash.asm) on the
// board that saves by reflashing its own PRG-ROM instead of battery WRAM:
//   node tools/make-u512-sample.js [dir]
//
// The third save-check fixture (CLAUDE.md's "fixtures, deliberately"
// note), and the same 2x1 walk as sample-mmc1/ and sample-mmc3/ -- same
// saver at the same coordinates running the same page, same title map, same
// (112, 112) start, same PLAYER_SPEED 2. What differs across the three is
// only the thing each one exists to prove: MMC1 and MMC3 differ in *register
// encoding* for the same battery-WRAM medium, but this board differs in the
// *save medium itself* (flash, not battery), so it earns a fixture on that
// basis alone, independent of what MMC1/MMC3 already cover between them.
//
// Deliberately has no opening `Say` the way sample-mmc3/ does. MMC3's Say
// exists because that board is the scanline-IRQ one, and a message box is
// what puts the font split to work during real gameplay rather than only on
// the title -- UNROM 512 has no split, so a Say here would exercise nothing
// this board doesn't already share with MMC1, and would only mean
// save_flash.lua needed MMC3 fixture's extra dialogue-dismissal phases for
// no reason. Do not "fix" this fixture to match MMC3's shape -- it is
// deliberately the MMC1 shape, mapper swapped.
...
const target = process.argv[2] ?? path.resolve(__dirname, '../sample-u512');
...
// to each other along rows 5-8 -- the same doorway idiom tools/make-sample.js
// uses. This is what gives the save round trip a screen to leave from that is
...
const project = createProject('UNROM 512 Save Sample');
project.cartridge.mapper = 30; // UNROM 512 -- the flash-save board
...
project.sprites = {
  ...
  actors: [
    ...
    // Stands still and is talked to by walking into it, same as sample/'s NPCs.
...
// The saver: touch triggers setSwitch, setVar, give (the gem) and Save --
// exactly the state a wrong restore would make visible (a switch, a
// variable, an inventory item and a screen that is not the start screen).
//
...
// and the page re-runs a frame later, handing out a second gem. Without the
...
console.log(`wrote UNROM 512 sample project to ${target}`);
