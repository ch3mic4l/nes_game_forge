#!/usr/bin/env node
// Writes the MMC1 sample project -- a durable, repeatable fixture (not a
// mkdtemp throwaway) exercising battery-backed save on the serial-port board:
//   node tools/make-mmc1-sample.js [dir]
//
...
const target = process.argv[2] ?? path.resolve(__dirname, '../sample-mmc1');
...
const project = createProject('MMC1 Save Sample');
project.cartridge.mapper = 1; // MMC1 -- the serial-port board, battery-capable
...
console.log(`wrote MMC1 sample project to ${target}`);
