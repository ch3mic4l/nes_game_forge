...
; `.if CAMERA_ENABLED` code still reserves the chain. The phase 2 slide is
; that consumer now, adding no RAM of its own and moving no symbol here.
; Chained after bt_walk_step, unconditionally.
cam_x_lo      = bt_walk_step+1   ; horizontal scroll, 0-255 -- mainline-owned,
                                  ; written by redraw_screen_slide (the arm's
                                  ; own reset to 0), camera_slide_tick and
                                  ; camera_slide_complete_b (the completion
                                  ; redraw's own reset back to 0, engine/
                                  ; camera.asm)
cam_y_lo      = cam_x_lo+1       ; vertical scroll, 0-239 -- mainline-owned,
                                  ; the identical writers as cam_x_lo
cam_nt        = cam_y_lo+1       ; PPUCTRL bits 0-1 -- mainline-owned, written
                                  ; by redraw_screen_slide, camera_slide_tick
                                  ; and camera_slide_complete_b
cam_slide_left = cam_nt+1        ; frames left in an active slide; 0 = idle.
                                  ; Armed by redraw_screen_slide, counted down
                                  ; by camera_slide_tick (engine/camera.asm)
cam_slide_dir = cam_slide_left+1 ; the DIR_* the active slide animates,
                                  ; written once by redraw_screen_slide
cam_far       = cam_slide_dir+1  ; resolved far-nametable index, staged by
                                  ; redraw_screen_slide across the slide's
                                  ; own draw
...
; for the coherent-stale-frame policy it protects. Written only by
; camera_slide_tick's own four wrap branches (engine/camera.asm).
cam_dirty     = cam_far+1
...
; Non-zero means an active slide still owes its completion redraw once
; cam_slide_left reaches zero -- set by redraw_screen_slide's own arm,
